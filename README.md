...
op, ctx := bedrock.Operation(ctx, "test",
...
1. **Initialization**:
   - `MustInit()` → `Init(ctx)` returns `(ctx, close)`
   - No more global bedrock instance
   - Explicit cleanup function

2. **Operations**:
   - `StartOperation()` → `Operation()` returns `(*Op, ctx)`
   - Context is now returned and must be used
   - `SetAttr()` → `Register()` with more flexible types

3. **Attributes**:
   - Use `attr.Error(err)` to mark failures
   - Support for events: `attr.NewEvent(name, ...attrs)`
   - Aggregations: `attr.Sum()`, `attr.Gauge()`, `attr.Histogram()`

4. **Metrics**:
   - Duration now in milliseconds (was seconds)
   - Explicit metric label registration required
   - Static labels automatically included

5. **New Features**:
   - Sources for long-running processes
   - Steps for lightweight tracing
   - Canonical logging for operation lifecycle
   - Convenient logging/metrics APIs
   - HTTP middleware with DoS protection

6. **Sources** (new concept):
```go
source, ctx := bedrock.Source(ctx, "worker")
defer source.Done()

source.Aggregate(ctx, attr.Sum("items", 1))
```
...
8. **Convenient APIs** (new):
```go
// Logging
bedrock.Info(ctx, "message", attr.String("key", "value"))

// Metrics
counter := bedrock.Counter(ctx, "requests_total", "Total requests")
counter.Inc()
```
